...
public class XIII1{
	...
	private static void question2(PrintStream out) {
        ...
        ProductsReader staticReader = new StaticTextProductsReader();
        market.loadProducts(staticReader);

        // Load products from file
        ProductsReader fileReader = new FileTextProductsReader("products.txt");
        market.loadProducts(fileReader);

        out.println("--- All Products :");
        out.println(market.allAlugados());

        Client u1 = new Client("101", "John Doe");
        Client u2 = new Client("202", "Jane Smith");

        market.share("ZA11ZB", u1);
        market.share("BB44ZB", u2);
	...
	private static void question3(PrintStream out) {
		...
		shareIt.share("AA22BB", u2); 	// true
		out.println("Shared products: " + shareIt.totalSharedProducts());

		shareIt.share("BB44ZB", u2); 	// true
		out.println("Shared products: " + shareIt.totalSharedProducts());

		shareIt.share("BB44ZB", u3); 	// false - já foi emprestado
		out.println("Shared products: " + shareIt.totalSharedProducts());

		shareIt.giveBack("BB44ZB");  	// true
		out.println("Shared products: " + shareIt.totalSharedProducts());

		shareIt.giveBack("BB44ZB");  	// false - não está emprestado
		out.println("Shared products: " + shareIt.totalSharedProducts());

		shareIt.giveBack("AA22BB");  	// true
		out.println("Shared products: " + shareIt.totalSharedProducts());
	}
}
