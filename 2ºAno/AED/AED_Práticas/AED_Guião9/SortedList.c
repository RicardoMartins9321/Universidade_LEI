...
struct _ListNode {
  void* item; // pointer to the item void* because that way it can be of any type
...
List* ListCreate(compFunc compF) {
  ...
  if (l == NULL) abort();
  l->size = 0;
  l->head = NULL; // the head of the list is the first node
  l->tail = NULL; // the tail of the list is the last node
  l->current = NULL;
...
void ListDestroy(List** p) {
  assert(*p != NULL);
  List* l = *p; // get the list from the pointer 
  ListClear(l); // clear the list 
...
void ListClear(List* l) {
  struct _ListNode* p = l->head;  // start at head of the Sortedlist, it's not the head of current node 
  struct _ListNode* aux;        // auxiliary pointer to node

  while (p != NULL) {
    aux = p;      // save pointer to current node
    p = aux->next;  // move to next node (if any)
...
void ListMove(List* l, int newPos) {
  ...
  if (newPos == -1) {  // move outside
    ...
    l->current = l->head;
    for (int i = 0; i < newPos; i++) l->current = l->current->next;
  }
  l->currentPos = newPos;
}
...
// The two loops in the ListSearch function serve different purposes:
// 1.The outer loop traverses the list from the current node (or the head if the current node is NULL)
// to the end of the list, looking for a node where the item matches the search key p.
// 2. If such a node is found, the inner loop starts from the head of the list and counts the number of 
// steps needed to reach the found node. This is necessary to update l->currentPos correctly.

// The reason for this two-loop structure is that in a singly linked list, we don't have a direct way to 
// know the position of a node (i.e., how many steps it is from the head) unless we count from the start of 
// the list. So, after finding the desired node with the outer loop, we use the inner loop to determine its position.

int ListSearch(List* l, const void* p) {
  struct _ListNode* temp = l->current ? l->current : l->head; // start from current node if exists, else start from head
  while(temp != NULL) {
    if (l->compare(temp->item, p) == 0) { // assuming compare is a function that compares the data
      l->current = temp;  // move current to found node
      l->currentPos = 0;  // reset current position
      struct _ListNode* countNode = l->head; // start from head to count position
      while(countNode != temp) { // loop until we reach the found node
        l->currentPos++; // increment current position
        countNode = countNode->next; // move to next node
      }
      return 0; // search successful
    }
    temp = temp->next;
  }
  return -1; // search unsuccessful, current remains unchanged
}
...
void* ListRemoveCurrent(List* l) {
  ...
  if (l->currentPos == 0)
    ...
    struct _ListNode* prevNode = l->head;
    while(prevNode->next != l->current) {
