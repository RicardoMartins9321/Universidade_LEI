...
static void checkInAtReception(int id)
{
    // entra na zona critica se o rececionista estiver disponível
    ...
    sh->fSt.receptionistRequest.reqGroup = id;  // guarda o id do grupo que está a pedir uma mesa
    ...
    // rececionista fica livre para poder receber pedidos, logo faz semUp do semáforo
    if (semUp (semgid, sh->receptionistReq) == -1) {                                                 
     perror ("error on the down operation for semaphore access (WT)");
        exit (EXIT_FAILURE);
    }

    // grupo espera que esteja disponível uma mesa
...
static void orderFood (int id)
{
    // entra na zona critica se o empregado de mesa estiver disponível
    ...
    sh->fSt.waiterRequest.reqGroup = id;    // guarda o id do grupo que está a pedir comida
    saveState(nFic, &sh->fSt);
    ...
    // empregado de mesa fica livre para poder receber pedidos, logo faz semUp do semáforo
...
static void waitFood (int id)
{
    int tableId = sh->fSt.assignedTable[id]; // guarda o id da mesa que o grupo está a ocupar
    ...
    // grupo espera que o empregado de mesa lhe traga a comida
...
static void checkOutAtReception (int id)
{
    ...
    sh->fSt.receptionistRequest.reqGroup = id;  // guarda o id do grupo que está a pedir a conta
    saveState(nFic, &sh->fSt);
    ...
    // grupo espera que o rececionista lhe dê a conta
