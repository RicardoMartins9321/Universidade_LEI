...
static request waitForClientOrChef()
{
    ...
    // empregado de mesa espera por um pedido de comida de um grupo ou do cozinheiro
    ...
    // empregado de mesa recebe o pedido de comida de um grupo ou do cozinheiro
    ...
    // empregado de mesa informa que pode receber um novo pedido
...
static void informChef (int n)
{
    ...
    sh->fSt.st.waiterStat = INFORM_CHEF;    // atualiza o estado do empregado de mesa
    sh->fSt.foodGroup = n;  // guarda o id do grupo que pediu comida
    saveState(nFic, &sh->fSt);
    ...
    // libera o cozinheiro para que este possa receber outros pedidos
    if (semUp (semgid, sh->waitOrder) == -1)  {                                                  
        perror ("error on the up operation for semaphore access (WT)");
        exit (EXIT_FAILURE);
    }

    // informa o grupo que o pedido foi recebido
    if (semUp (semgid, sh->requestReceived[tableId]) == -1)  {                                                  
        perror ("error on the up operation for semaphore access (WT)");
        exit (EXIT_FAILURE);
    }

    // empregado de mesa espera que o cozinheiro receba o pedido
...
static void takeFoodToTable (int n)
{
    ...
    // faz com que o grupo saiba que a comida está pronta
