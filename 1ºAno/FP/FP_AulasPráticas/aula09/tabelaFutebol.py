...
def numJogos(reg): return reg[V]+reg[E]+reg[D]
...
def pontos(reg): return 3 * reg[V] + reg[E] * 1
...
def main():
    ...
    tab = sorted(
        tabela, key=lambda tabela: tabela[GM]-tabela[GS], reverse=True)
    printTabela(tab)
    ...
    tab = sorted(tabela, key=lambda tabela: (
        pontos(tabela), tabela[GM]-tabela[GS]), reverse=True)
    printTabela(tab)
