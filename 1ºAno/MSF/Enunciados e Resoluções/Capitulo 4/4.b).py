...
w = 100
#w_x_v = np.cross([0, 0, 100], [v0x, v0y, v0z])
#print(w_x_v)
m = 0.057
ro_ar = 1.225
raio = 0.0335
...
for dt in [0.1,0.01,0.001,0.0001,0.00001]:			# experimentar várias passos de tempo
    ...
    for i in range(n2):
        ...
        # fazendo o produto vetorial de w x v dá que amx = -vy*w
        # e que aceleração_magnus de y = vx*w
        aceleracao_magnusx = (-f_magnus * w * vy2[i])/m
        aceleracao_magnusy = (f_magnus * w * vx2[i])/m

        ax2[i] = -dres*vv*vx2[i] + aceleracao_magnusx
        ay2[i] = -g-dres*vv*vy2[i] + aceleracao_magnusy
        az2[i] = 0
