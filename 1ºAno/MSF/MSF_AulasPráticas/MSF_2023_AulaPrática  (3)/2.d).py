...
aceleracao_alternativa = 9.8 - 9.8/6.8**2 * (6.8*np.tanh(10*tempo/6.8)) * abs((6.8*np.tanh(10*tempo/6.8)))
...
plt.subplot(1, 2, 2)
plt.plot(tempo, aceleracao_alternativa)
plt.scatter(tempo, aceleracao, color="orange")
plt.title("Lei do Movimento")
plt.xlabel("Tempo (s)")
plt.ylabel("Aceleração (m/s^2)")
plt.grid()
